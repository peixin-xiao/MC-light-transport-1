...
struct Tissue {
  ...
  __device__ thrust::tuple<float(*)[2]> bubbleSort(float arr[N][2], int FLAG) {
    ...
    float (*ar)[2];
    cudaMalloc((void**)&ar, N * 2 * sizeof(float));
  ...
  __device__ thrust::tuple<float, float, float> refra(float face1[], float face2[]
      ...
      di[0] = ux;/*light incident direction*/
      di[1] = uy;
      di[2] = uz;

      n1[0] = d1[1] * d2[2] - d1[2] * d2[1];
      n1[1] = d1[2] * d2[0] - d1[0] * d2[2];
      n1[2] = d1[0] * d2[1] - d1[1] * d2[0];//normal line of the face : n1 = d1 × d2
      //----------------------------------------------------
      tempn1 = pow(n1[0], 2) + pow(n1[1], 2)
          + pow(n1[2], 2);
      ...
      n2[0] = n1[1] * di[2] - n1[2] * di[1];
      n2[1] = n1[2] * di[0] - n1[0] * di[2];
      n2[2] = n1[0] * di[1] - n1[1] * di[0];/*normal line between the light direction and
      ...
      //-----------------------------------------------------
      theta1 = acos(n1[0] * di[0] + n1[1] * di[1] + n1[2] * di[2]);
      ...
      temp0[0] = n2[1] * di[2] - n2[2] * di[1];
      temp0[1] = n2[2] * di[0] - n2[0] * di[2];
      temp0[2] = n2[0] * di[1] - n2[1] * di[0];/* temp = n2 × di*/

      temp1 = n2[0] * di[0] + n2[1] * di[1] + n2[2] * di[2];
      temp2[0] = n2[0] * temp1;
      temp2[1] = n2[1] * temp1;
      temp2[2] = n2[2] * temp1;
...
unsigned int Rand1(unsigned int randx)
{
 ...
 __device__ thrust::tuple<float, bool, int, float, int, int,int,float (*)[K][5]>distance_to_interface(float x,
    ...
    float length_voxel, Tissue a[2],int time,float (*co)[K][5], bool reset,int idx) {
 ...
 __global__ void simulate(short int* T, int Nbins, float length_voxel, Tissue a[2], int FLAG,
    ...
    if(i<M) {
        ...
        thrust::tie(path2_gpu) = montecarlo(T, Nbins, length_voxel, a, FLAG,
            vertex, path2_gpu, c0, rndseed_gpu[i], i);
...
int main()
{
    ...
    a[1] = { 1,"medium 1",0.1,0.245e4,0.7 ,1.333 };//mua ~= 0.001/cm
    ...
    for (j = 0;j < (AM / M);j++) {
        ...
        simulate << <1, M >> > (T1_gpu, Nbins, length_voxel,
            a_gpu, FLAG, vertex_gpu, path2_gpu, c0, rndseed_gpu);
