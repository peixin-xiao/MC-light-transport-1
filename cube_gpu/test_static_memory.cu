...
std::tuple<float (*)[5] > record_path(float(*c0)[5], int k, int reset) {
    //static int k0 = 0;
...
std::tuple< float (*)[K][5] >  montecarlo
    ...
    do {
        ...
        do {
            ...
            if (sleft == 0 || flag1 == 1) {
                ...
                std::tie(path0) = record_path(c0, k, 0);
...
int main()
{
    ...
    for (i = 0;i < int(AM / M);i++) {
        ...
        std::tie(path1) = montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex, 0);
        montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex, 1);
