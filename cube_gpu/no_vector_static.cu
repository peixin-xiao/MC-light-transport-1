...
#define M 1//amount of a batch of photons
#define AM 10
...
thrust::tuple<float(*)[2]> bubbleSort(float arr[N][2], int FLAG) {
    //float(*a)[2];
...
thrust::tuple<float, float, float> refra(float face1[], float face2[]
...
thrust::tuple<int,float,float*> randnum(int n,float rnd[COUNT],bool type) {
...
thrust::tuple<float, bool, int, float, int, int,int,float (*)[5]>distance_to_interface(float x,
...
thrust::tuple<float(*)[5] ,int> record_path(float(*c0)[5], 
...
thrust::tuple< float(*)[K][5] >  montecarlo
(short int* T, int Nbins, float length_voxel, Tissue a[2], int FLAG, int(*vertex)[3], bool reset) {
    ...
    return thrust::make_tuple(path1);
...
int main()
{
    ...
    //std::tie(path) = montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex,0);
    write_path(directory1);
    for (i = 0;i < int(AM / M);i++) {
        //montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex, 1);
        thrust::tie(path1) = montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex, 0);
        montecarlo(T1, Nbins, length_voxel, a, FLAG, vertex, 1);

        tie(path) = array2vector(path1, 0);
        /*std::cout << "path1.time : \n";
        for (i = 0;i < 1000;i++) {
            std::cout  << path1[0][i][0]<<"\n";
        }*/
        write_path2(path, directory1);
        array2vector(path1, 1);
    }
