...
#define M 50//amount of a batch of photons
#define AM 10
...
(short int* T, int Nbins, float length_voxel, Tissue a[2], int FLAG, 
    int(*vertex)[3], float (*path2_gpu)[K][5],float (*c0)[K][5],float *rand,int rndseed,int idx) {
 ...
 __global__ void simulate(short int* T, int Nbins, float length_voxel, Tissue a[2], int FLAG,
    int(*vertex)[3], float(*path2_gpu)[K][5], float(*c0)[K][5],float *rand,
     int *rndseed_gpu) {
    ...
    if(i<M) {
        ...
        thrust::tie(path2_gpu) = montecarlo(T, Nbins, length_voxel, a, FLAG,
            vertex, path2_gpu, c0, rand, rndseed_gpu[i], i);
...
int main()
{
   ...
   simulate << <1, M>> > (T1_gpu, Nbins, length_voxel,
        a_gpu, FLAG,vertex_gpu,path2_gpu,c0 ,rand,rndseed_gpu);
