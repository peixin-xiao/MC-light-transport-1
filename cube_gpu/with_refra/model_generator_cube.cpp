    ...
    for (i = 0; i < Nbins;i++) {
        for (j = 0;j < Nbins;j++) {
            for (k = 0;k < Nbins;k++) {
                ...
                flag = f % 3;
                if (((i<sphere_center[0]+10)&&(i>sphere_center[0]-10))&& 
                    ((j < sphere_center[1] + 10) && (j > sphere_center[1] - 10))&&
                    ((k < sphere_center[2] + 10) && (k > sphere_center[2] - 10))) {
...
short int* create3Dmodel_lr(int Nbins) { //lr: low resolution
    ...
    for (i = 0; i < Nbins;i++) {
        for (j = 0;j < Nbins;j++) {
            for (k = 0;k < Nbins;k++) {
                ...
                flag = f % 3;
                if (((i < sphere_center[0] + 1) && (i > sphere_center[0] - 1)) &&
                    ((j < sphere_center[1] + 1) && (j > sphere_center[1] - 1)) &&
                    ((k < sphere_center[2] + 1) && (k > sphere_center[2] - 1))) {
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 1;
...
{
    int Nbins = Nb;
    float length_voxel = 0.0005;
    int FLAG;
    int(*p)[3];
    char directory[] = "C:\\Users\\Administrator\\source\\data\\model.dat";
    short int* T = create3Dmodel(Nbins);
    short int* T_lr = create3Dmodel_lr(int(Nbins/10));
    std::tie(FLAG, p) = find_vertex(int(Nbins / 10), T_lr);
