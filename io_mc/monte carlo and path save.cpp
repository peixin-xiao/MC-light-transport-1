// monte carlo and path save.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
std::tuple<int , float ,short int*> read_model(std::string directory) {
    int i, j, k;
    int Nbins;
    float length_voxel;
    short int* T;
    ...
    fin.close();
    return std::make_tuple(Nbins, length_voxel,T);
...
(short int* T, int Nbins,float length_voxel, Tissue a[2]) {
...
int main()
{
    ...
    printf("success04\n");
    short int* T1;
    std::tie(Nbins,length_voxel,T1) = read_model(directory0);
    tie(q, path) = montecarlo(T1, Nbins,length_voxel, a);
